...
//// 이진표면을 비트맵핸들로 변환한다 
HBITMAP __geSurface2BmpHandle(BIN_SURFACEINFO* psInfo)
{
	HBITMAP hBit = 0;
	BITMAPINFO BitmapInfo;

	__geSetBitmapinfoBin(psInfo, &BitmapInfo, TRUE);
	...
	memcpy(pSurface, psInfo->pSurface, BitmapInfo.bmiHeader.biSizeImage);
	return hBit;
}
...
BOOL __gePutSprite(HDC dcDst, int x, int y, SURFACEINFO* psInfo, HDC hTempMemDC)
{
	return TransparentBlt(dcDst,
		x, y, psInfo->nWidth, psInfo->nHeight,
		hTempMemDC,
...
BOOL __gePutStretch(HDC dcDst, int x, int y, SURFACEINFO* psInfo)
{
	return StretchBlt(dcDst,
		x + psInfo->nWidth, y, -(psInfo->nWidth), psInfo->nHeight,
		psInfo->dcSurface,
		0, 0, psInfo->nWidth, psInfo->nHeight,
		SRCCOPY);
}
