...
void DrawRoundRect(HWND hWnd, int x1, int y1, int x2, int y2, BOOL bFill, int nPenStyle, int nHatchBrush, int nWidth, COLORREF rgbPen, COLORREF rgbBrush){
    ...
    if (bFill) {
        ...
        SetROP2(hdc, R2_NOT);
    maxRound = ((x2 - x1) > (y2 - y1)) ? (x2 - x1) / 10 : (y2 - y1) / 10;
    RoundRect(hdc, x1, y1, x2, y2, maxRound, maxRound);
